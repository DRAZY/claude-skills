...
### New Skills
- **`/blog-writer`** (v1.4.0) — A **universal** long-form article writer for **any field or topic** (tech, science, business, health, finance, culture, personal, and more) — not security-specific; a technical/security walkthrough is just one of its ten archetypes, used when the topic calls for it. **Matches the writer's voice** from a pasted sample or style file (reads for sentence rhythm, vocabulary, openings, hedging, and signature tics) so posts sound like the author, not generic AI. Adds **skimmable web formatting** (descriptive subheads, bolded key lines, TL;DRs, `[IMAGE:]`/`[DIAGRAM:]` placement cues, hero-image concept) and **first-class revision** (treats the first draft as a draft, invites feedback, revises surgically with the voice locked). Now a seven-stage process (interview → voice → research → outline → draft → edit → revise). **Interviews the writer first** — given a topic or rough angle, it asks 3–6 sharp, topic-specific clarifying questions (the core point, audience, hook, the writer's first-hand experience, scope, publishing target) and waits for answers before drafting, so the piece reflects the writer's actual intent rather than assumptions (with a "just go" escape hatch that proceeds on best read and states its assumptions). Then runs a real five-stage process (angle → research with source verification → outline → full draft → self-edit), not a one-pass skeleton. Ships **10 post archetypes** (tutorial, security walkthrough / vulnerability writeup, explainer, opinion, market / industry analysis, review, case study, news analysis, listicle, personal essay), **length tiers** (short ~600 to definitive 4,000+, depth over word count), **personal vs work voice** profiles, and a **responsible-disclosure lens** for security research (mechanism + detection + fix, canaries over live payloads, no turnkey exploits — hands off to `/prompt-injection-probe`, `/vuln-triage`, `/disclosure-writer` where the work belongs). Includes a dedicated **anti-AI-slop edit pass** (strips "in today's landscape," corporate vocabulary, robotic three-part rhythm, restate-the-intro conclusions, em-dash overuse), WebSearch fact verification (never invents a source), clean CommonMark output, and word count / dated sources / headline options / an honest weak-spot note.
